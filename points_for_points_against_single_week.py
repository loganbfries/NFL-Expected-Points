...
def save_data_to_df(teams, nfl_points_dict):
    save_path = "/Users/loganfries/iCloud/SportsAnalytics/NFL/Data/"
    ...
    df = pd.DataFrame(
        ...
        columns=[
            "Team",
            "Avg Points For Week {week}".format(week=week),
            "Avg Points For Against {week}".format(week=week),
        ],
    )

    df.to_csv(
        save_path + "points_for_points_against_week_{week}.csv".format(week=week),
        index=False,
    )
...
save_data_to_df(teams, nfl_points_dict)
